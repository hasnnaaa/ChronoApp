...
import { Loader2, Image as ImageIcon, X, Save, Upload } from 'lucide-react'
...
export default function EditPost() {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      ...
      try {
        ...
        if (data.image_url) setIsImageSet(true)
  ...
  if (loading) return <div className="text-center py-20 text-slate-400">Mengambil data...</div>

  return (
    <div className="max-w-6xl mx-auto px-4 py-8 md:py-10 pb-24">
      
      <div className="mb-8 md:mb-10 text-center">
        <h1 className="font-logo font-bold text-3xl md:text-4xl mb-2 text-slate-900">Edit Story.</h1>
        <p className="text-sm md:text-base text-slate-500">Perbaiki tulisanmu agar lebih sempurna.</p>
      </div>
      ...
      <form onSubmit={handleUpdate} className="space-y-6 md:space-y-8">
        ...
        <div className="min-h-[300px] md:min-h-[400px]">
          <ReactQuill 
            ...
            onChange={setContent} 
            modules={modules} 
            className="h-[250px] md:h-[350px] font-serif text-base md:text-lg mb-12" 
          />
        </div>

        {/* SAVE BUTTONS */}
        <div className="flex justify-end pt-10 border-t border-slate-100 gap-4">
          <button 
            type="button" 
            onClick={() => navigate('/profile')} 
            className="px-6 py-3 text-slate-500 font-bold hover:text-black transition-colors text-sm md:text-base"
          >
            Batal
          </button>
          ...
          <button 
            type="submit" 
            disabled={updating} 
            className="px-8 py-3 bg-black text-white rounded-full font-bold hover:bg-slate-800 transition-all flex items-center gap-2 shadow-lg disabled:opacity-50"
          >
